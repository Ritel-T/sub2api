...
class ReleaseMatrixTest(unittest.TestCase):
    def test_four_number_fork_version_archive(self):
        target = {'goos': 'linux', 'goarch': 'amd64'}
        self.assertEqual(
            release.archive_name('2.8.15.1-ritel', target),
            'sub2api_2.8.15.1-ritel_linux_amd64.tar.gz')
        with self.assertRaises(ValueError):
            release.archive_name('2.8.15.1.2-ritel', target)
