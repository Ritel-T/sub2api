  ...
  <AppLayout>
    <div class="mx-auto max-w-4xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <!-- Simulated Cache Settings -->
        <div class="card">
          <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ t('admin.settings.simCache.title') }}
            </h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {{ t('admin.settings.simCache.description') }}
            </p>
          </div>
          <div class="space-y-5 p-6">
            <div v-if="simCacheLoading" class="flex items-center gap-2 text-gray-500">
              <div class="h-4 w-4 animate-spin rounded-full border-b-2 border-primary-600"></div>
              {{ t('common.loading') }}
            </div>
            ...
            <template v-else>
              <div class="flex items-center justify-between">
                <div>
                  <label class="font-medium text-gray-900 dark:text-white">{{
                    t('admin.settings.simCache.enabled')
                  }}</label>
                  <p class="text-sm text-gray-500 dark:text-gray-400">
                    {{ t('admin.settings.simCache.enabledHint') }}
                  </p>
                </div>
                <Toggle v-model="simCacheForm.enabled" />
              </div>

              <div
                v-if="simCacheForm.enabled"
                class="space-y-4 border-t border-gray-100 pt-4 dark:border-dark-700"
              >
                <div>
                  <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
                    {{ t('admin.settings.simCache.missProbability') }}
                  </label>
                  <input
                    v-model.number="simCacheForm.miss_probability"
                    type="number"
                    min="0"
                    max="1"
                    step="0.01"
                  ...
                  />
                  <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                    {{ t('admin.settings.simCache.missProbabilityHint') }}
                  </p>
                </div>

                <div>
                  <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
                    {{ t('admin.settings.simCache.ttlSeconds') }}
                  </label>
                  <input
                    v-model.number="simCacheForm.ttl_seconds"
                    type="number"
                    min="1"
                    max="3600"
                    class="input w-32"
              ...
              <div class="flex justify-end border-t border-gray-100 pt-4 dark:border-dark-700">
                <button
                  type="button"
                  @click="saveSimCacheSettings"
                  :disabled="simCacheSaving"
                  class="btn btn-primary btn-sm"
                >
                  <svg
                    v-if="simCacheSaving"
                  ...
                  {{ simCacheSaving ? t('common.saving') : t('common.save') }}
                </button>
              </div>
            </template>
          </div>
...
// Simulated Cache 状态
const simCacheLoading = ref(true)
const simCacheSaving = ref(false)
...
// Simulated Cache 方法
async function loadSimCacheSettings() {
  simCacheLoading.value = true
  try {
    const settings = await adminAPI.settings.getSimCacheSettings()
    Object.assign(simCacheForm, settings)
  } catch (error: any) {
    console.error('Failed to load simulated cache settings:', error)
  } finally {
    simCacheLoading.value = false
  }
}

async function saveSimCacheSettings() {
  simCacheSaving.value = true
  try {
    ...
    appStore.showError(
      t('admin.settings.simCache.saveFailed') + ': ' + (error.message || t('common.unknownError'))
    )
  } finally {
    simCacheSaving.value = false
  }
}
